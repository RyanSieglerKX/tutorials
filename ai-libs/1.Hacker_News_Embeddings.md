...
.walkthroughUtil.queryPartitionHnsw:{[q;sd;ed;wc;k;metric;efs]
    ...
    }[res;q;k;metric;efs;]peach dts;
    op:(neg;1*)@(metric~`L2);
    k:op k;
    tab:tab@.ai.tss.utils.topK[tab`dist;k][::;1]
 }
```
In the above:
...
```q
1 sublist select title, score from news where date within (2022.01.01; 2023.01.01)
```
    title                   score
    -----------------------------
    Intellectual Loneliness 386 
...
In your Python code, import pykx and load the necessary kdb+/q scripts, where the `.walkthroughUtil.queryPartitionHnsw` is defined in `utils.q`:
```bash
pip install pykx
```
```python
