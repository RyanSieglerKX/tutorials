...
4. Ensure [PyKX Under Q](https://code.kx.com/pykx/pykx-under-q/intro.html#prerequisites) is installed, you will likely need to run the below:
    ```bash
    pip install pykx~=3.2
    
    python -c "import pykx;pykx.install_into_QHOME(to_local_folder='your/kdb/path')"
    # replace with your/kdb/path which you can find by running the below
    # dirname "$(dirname "$(which q)")"
    ```
