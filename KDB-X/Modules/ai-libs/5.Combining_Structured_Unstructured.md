...
2. Ensure Python for KDB-X (`pykx`) is installed:
    ```bash
    pip install pykx==4.0.0.b1
    ```
...
Next you will need to make sure [Python for KDB-X invoked under Q](https://code.kx.com/pykx/pykx-under-q/intro.html#prerequisites) is installed by running:
...
Relevant Python for KDB-X concepts used in the above include:
