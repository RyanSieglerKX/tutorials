...
Next download the the [stocks.txt](https://github.com/KxSystems/tutorials/blob/main/KDB-X/Python/stocks.txt) file locally and make sure the path below points to it.
...
It is important to note that the aggregation below is running over on-disk data rather than on in-memory objects as this showcases the power and efficiency of KDB-X Python. 
